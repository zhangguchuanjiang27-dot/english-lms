    ...
    return (
        <div className="w-full max-w-2xl mx-auto px-4 py-12 flex flex-col items-center">
            ...
            <motion.div
                ...
                className="bg-indigo-900/40 backdrop-blur-xl border border-indigo-500/30 p-8 rounded-[3rem] w-full shadow-2xl relative overflow-hidden mb-8"
            >
                <div className="text-center mb-10">
                    <p className="text-xs font-black uppercase tracking-widest text-indigo-400 mb-1">獲得経験値</p>
                    <p className="text-6xl font-black text-white">{score} <span className="text-2xl text-indigo-300">EXP</span></p>
                </div>
                ...
                <div className="space-y-4">
                    <div className="flex justify-between items-end px-2">
                        <div className="flex flex-col">
                            <span className="text-xs font-bold text-indigo-400 uppercase tracking-tighter">Level</span>
                            <span className="text-3xl font-black text-white">{currentLevel}</span>
                        </div>
                        <div className="text-right">
                            <span className="text-sm font-bold text-indigo-200">{Math.floor(displayXP)} / {displayRequired} XP</span>
                        </div>
                    </div>

                    <div className="h-6 w-full bg-indigo-950 rounded-full p-1 border border-indigo-800/50 relative overflow-hidden">
                        <motion.div
                ...
                <AnimatePresence>
                    {isLevelingUp && (
                        <motion.div
                            ...
                            className="absolute inset-0 flex items-center justify-center bg-indigo-600/20 backdrop-blur-sm z-20 rounded-[3rem]"
                        >
                            <div className="text-center">
                                ...
                                >
                                    <Star size={80} className="text-amber-400 fill-amber-400 mx-auto mb-4" />
                                </motion.div>
                                <h2 className="text-5xl font-black text-white italic tracking-tighter drop-shadow-lg">LEVEL UP!</h2>
                            </div>
                        </motion.div>
                    )}
                </AnimatePresence>
            ...
            {showButtons && (
                ...
                >
                    <button
                        onClick={onBack}
                        className="flex-1 py-4 bg-slate-800 hover:bg-slate-700 text-white rounded-2xl font-black transition-all flex items-center justify-center gap-2"
                    >
                    ...
                    <button
                        onClick={onRetry}
                        className={cn(
                            "flex-[2] py-4 rounded-2xl font-black shadow-lg transition-all active:scale-95 flex items-center justify-center gap-2",
                            wrongAnswersCount > 0
